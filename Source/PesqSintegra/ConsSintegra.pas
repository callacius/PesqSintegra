...
begin
  ...
  begin
    Result := trim(Copy(Value,Pos('Identificação do Estabelecimento',Value),Value.Length));
  end
  else if (UF='PR') then
  begin
  ...
  begin
    Result := trim(Copy(Value,Pos('Identificação',Value),Value.Length));
  end
  else if (UF='RECEITA') then
  begin
    Result := trim(Copy(Value,Pos('CADASTRO NACIONAL DA PESSOA JURÍDICA',Value),Value.Length));
  end
  else
  begin
    Result := Value;
...
//    20 : FClassUF := TClassUF_RJ.Create;
    ...
    begin
      ShowMessage('Desculpe! Não foi possível encontrar o UF Informado');
      Abort;
    end;
  end;
end;
